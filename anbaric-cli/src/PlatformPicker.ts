import {DEFAULT_PLATFORM_URL, PRODUCTION_PLATFORM_URL, STAGING_PLATFORM_URL} from "./CliConfig";
...
const choosePlatformUrl = async () : Promise<string> => {
    const environments = [
        { label: `Local Dev ${dim(`(${DEFAULT_PLATFORM_URL})`)}`, value: DEFAULT_PLATFORM_URL },
        { label: `Staging ${dim(`(${STAGING_PLATFORM_URL})`)}`, value: STAGING_PLATFORM_URL },
        { label: `Production ${dim(`(${PRODUCTION_PLATFORM_URL})`)}`, value: PRODUCTION_PLATFORM_URL },
    ];
    const connected = await Promise.all(environments.map(environment => ping(environment.value)));

    return select("Which platform do you want to use?", environments.map((environment, index) => ({
        ...environment,
