...
describe("ConsoleAuditor", () => {
    ...
    it("logs the resource, actor, interaction and description", async () => {
        ...
        await new ConsoleAuditor().audit("crm", "job", "job-1", new Human("chris", "admin"), ["UPDATE_PROPERTIES"], "Properties updated", { age: 42 });

        expect(log).toHaveBeenCalledExactlyOnceWith('[crm/job job-1] chris UPDATE_PROPERTIES Properties updated {"age":42}');
    });

    it("logs state changes made by the machine", async () => {
        const log = vi.spyOn(console, "log").mockImplementation(() => {});

        await new ConsoleAuditor().audit(undefined, "job", "job-1", new Code("workflow-1", "state-machine"), ["CHANGE_STATE"], 'Transitioned to "done"', null);
