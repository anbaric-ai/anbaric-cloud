...
describe("InMemoryQueue", () => {
    ...
    it("dequeues enqueued messages in FIFO order", async () => {
        await queue.enqueue("a", undefined, "workflow-1");
        await queue.enqueue("b", undefined, "workflow-2");
        await queue.enqueue("c", undefined, "workflow-1");
    ...
    it("releases scheduled messages once their due date has passed", async () => {
        await queue.schedule("a", undefined, "workflow-1", secondsFromNow(-1));

        expect(await queue.dequeueSome()).toEqual([message("a")]);
    });

    it("holds back messages scheduled for the future", async () => {
        await queue.schedule("a", undefined, "workflow-1", secondsFromNow(60));
    ...
    it("keeps future messages parked across drains", async () => {
        ...
        await queue.dequeueSome();
        await queue.enqueue("b", undefined, "workflow-1");

        expect(await queue.dequeueSome()).toEqual([message("b")]);
    });

    it("returns ready messages before released scheduled messages", async () => {
        await queue.schedule("late", undefined, "workflow-1", secondsFromNow(-1));
        await queue.enqueue("ready", undefined, "workflow-1");
    ...
    it("orders released messages by due date, earliest first", async () => {
        await queue.schedule("later", undefined, "workflow-1", secondsFromNow(-10));
        await queue.schedule("earliest", undefined, "workflow-1", secondsFromNow(-30));
        await queue.schedule("middle", undefined, "workflow-1", secondsFromNow(-20));
