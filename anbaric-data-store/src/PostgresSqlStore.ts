...
/* The cloud SQL backend. All apps in a tenant share one dedicated schema
   ...
   use and set as the search_path for every connection. The app id does not
   affect the schema; it is AppAware only so its writes are audited against the
   calling app. PostgreSQL uses numbered `$1, $2` placeholders. */
